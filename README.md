...
elacsym 是一个高性能、低成本的开源向量数据库，采用 Serverless 架构设计。主要特点：

- **存储与计算分离**：基于对象存储（S3/MinIO），实现按需加载索引
- **高效 ANN 算法**：使用 RaBitQ 量化技术，32x 内存压缩率，精度优于 PQ/SQ
- **自动扩展**：无状态查询执行器，支持水平扩展
- **低成本**：冷数据零成本，按实际计算资源付费
- **多租户隔离**：通过 Namespace 实现硬隔离

## 架构

elacsym 采用 Pinecone Serverless 的设计理念，核心组件包括：

```
API Gateway → Query Executor → Cache Layer → Blob Storage (S3/MinIO)
              ↓
         Freshness Layer (秒级新鲜度)
              ↓
         Metadata Service (RocksDB)
              ↓
         Index Builder (后台异步)
```
